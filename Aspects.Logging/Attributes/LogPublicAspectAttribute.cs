...
public class LogPublicAspectAttribute : AbstractLogAspectAttribute
{
    private readonly Lazy<ILogger<LogPublicAspectAttribute>?> _logger = AspectServiceLocator.GetService<ILogger<LogPublicAspectAttribute>>();

    public LogPublicAspectAttribute()
    {
    }

    public LogPublicAspectAttribute(string prefix) : base(prefix)
    {
    }

    public LogPublicAspectAttribute(LogOptions logOptions = DefaultLogOptions, string? prefix = null) : base(logOptions, prefix)
    {
    }

    public LogPublicAspectAttribute(LogPoint logPoint = DefaultLogPoint, LogOptions logOptions = DefaultLogOptions, string? prefix = null) : base(logPoint, logOptions, prefix)
    {
    }

    public LogPublicAspectAttribute(LogLevel logLevel, LogPoint logPoint = DefaultLogPoint, LogOptions logOptions = DefaultLogOptions, string? prefix = null) : base(logLevel, logPoint, logOptions, prefix)
    {
    }
